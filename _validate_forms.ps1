...
& $exe --profile $profile --command table-columns --object $testObject --outfile cli_columns.json 2>&1 | Out-Null

# Form: Extract columns tab
Write-Host "Getting Form data (columns tab)..." -ForegroundColor Cyan
& $exe --profile $profile --test-form table-details --object $testObject --tab columns --outfile form_columns.json 2>&1 | Out-Null

if ((Test-Path "cli_columns.json") -and (Test-Path "form_columns.json")) {
    $cliData = Get-Content "cli_columns.json" -Raw | ConvertFrom-Json
    $formData = Get-Content "form_columns.json" -Raw | ConvertFrom-Json
...
& $exe --profile $profile --command table-fks --object $testObject --outfile cli_fks.json 2>&1 | Out-Null

# Form: Extract FK tab
Write-Host "Getting Form data (foreign-keys tab)..." -ForegroundColor Cyan
& $exe --profile $profile --test-form table-details --object $testObject --tab foreign-keys --outfile form_fks.json 2>&1 | Out-Null

if ((Test-Path "cli_fks.json") -and (Test-Path "form_fks.json")) {
    $cliData = Get-Content "cli_fks.json" -Raw | ConvertFrom-Json
    $formData = Get-Content "form_fks.json" -Raw | ConvertFrom-Json
...
& $exe --profile $profile --command table-indexes --object $testObject --outfile cli_indexes.json 2>&1 | Out-Null

# Form: Extract indexes tab
Write-Host "Getting Form data (indexes tab)..." -ForegroundColor Cyan
& $exe --profile $profile --test-form table-details --object $testObject --tab indexes --outfile form_indexes.json 2>&1 | Out-Null

if ((Test-Path "cli_indexes.json") -and (Test-Path "form_indexes.json")) {
    $cliData = Get-Content "cli_indexes.json" -Raw | ConvertFrom-Json
    $formData = Get-Content "form_indexes.json" -Raw | ConvertFrom-Json
...
& $exe --profile $profile --command ddl --object $testObject --outfile cli_ddl.json 2>&1 | Out-Null

# Form: Extract DDL tab
Write-Host "Getting Form data (ddl-script tab)..." -ForegroundColor Cyan
& $exe --profile $profile --test-form table-details --object $testObject --tab ddl-script --outfile form_ddl.json 2>&1 | Out-Null

if ((Test-Path "cli_ddl.json") -and (Test-Path "form_ddl.json")) {
    $cliData = Get-Content "cli_ddl.json" -Raw | ConvertFrom-Json
    $formData = Get-Content "form_ddl.json" -Raw | ConvertFrom-Json
...
# Save results
$validationResults | Export-Csv "validation_results.csv" -NoTypeInformation
$validationResults | ConvertTo-Json -Depth 5 | Out-File "validation_results.json" -Encoding UTF8

Write-Host ""
Write-Host "📊 Results saved to:" -ForegroundColor Cyan
Write-Host "   • validation_results.csv" -ForegroundColor White
Write-Host "   • validation_results.json" -ForegroundColor White
Write-Host ""
Write-Host "=============================================" -ForegroundColor Cyan
