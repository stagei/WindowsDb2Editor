...
& $exe --profile $Profile --command list-tables --outfile tables_list.json 2>&1 | Out-Null

if (Test-Path "tables_list.json") {
    $tablesData = Get-Content "tables_list.json" -Raw | ConvertFrom-Json
...
foreach ($config in $testConfigs) {
    ...
    try {
        ...
        $cliFile = "test_cli_$($config.Name).json"
        ...
        if ($config.FormCommand) {
            ...
            $formFile = "test_form_$($config.Name).json"
...
# Save results
$results | ConvertTo-Json -Depth 5 | Out-File "continuous_validation_results.json" -Encoding UTF8

Write-Host "📊 Results saved to: continuous_validation_results.json" -ForegroundColor Cyan
Write-Host ""
Write-Host "=============================================" -ForegroundColor Cyan
