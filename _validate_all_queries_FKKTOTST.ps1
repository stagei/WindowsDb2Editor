...
function Test-Query {
    ...
    try {
        ...
        if ($CliCommand -and $CliCommand -ne "N/A") {
            $cliFile = "validation_cli_$($QueryName).json"
        ...
        if ($FormCommand -and $FormCommand -ne "N/A") {
            $formFile = "validation_form_$($QueryName).json"
...
& $exe --profile $Profile --command list-tables --outfile fkktotst_tables.json 2>&1 | Out-Null

if (Test-Path "fkktotst_tables.json") {
    $tablesData = Get-Content "fkktotst_tables.json" -Raw | ConvertFrom-Json
...
$csvFile = "validation_fkktotst_$timestamp.csv"
$jsonFile = "validation_fkktotst_$timestamp.json"
