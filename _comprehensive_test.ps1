...
foreach ($test in $coreCommands) {
    ...
    $outFile = "test_output_$testNum.json"
...
$tableListResult = & $exe --profile BASISTST --command list-tables --schema INL --outfile temp_tables.json 2>&1

if ($LASTEXITCODE -eq 0 -and (Test-Path "temp_tables.json")) {
    $tablesData = Get-Content "temp_tables.json" -Raw | ConvertFrom-Json
    if ($tablesData.tables -and $tablesData.tables.Count -gt 0) {
        ...
        foreach ($test in $tableCommands) {
            ...
            $outFile = "test_table_$(($test.cmd -replace '-','_')).json"
...
# Save results
$testResults | Export-Csv "test_results.csv" -NoTypeInformation
...
Write-Host "   • test_results.csv" -ForegroundColor White
Write-Host "   • test_results.json" -ForegroundColor White
Write-Host ""
Write-Host "=============================================" -ForegroundColor Cyan
