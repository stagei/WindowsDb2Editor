...
& $exe --profile $Profile --command list-tables --limit 10 --outfile _test_tables.json 2>&1 | Out-Null
& $exe --profile $Profile --command list-views --limit 5 --outfile _test_views.json 2>&1 | Out-Null
& $exe --profile $Profile --command list-procedures --limit 5 --outfile _test_procedures.json 2>&1 | Out-Null
& $exe --profile $Profile --command list-functions --limit 5 --outfile _test_functions.json 2>&1 | Out-Null
& $exe --profile $Profile --command list-triggers --limit 5 --outfile _test_triggers.json 2>&1 | Out-Null
& $exe --profile $Profile --command list-packages --limit 5 --outfile _test_packages.json 2>&1 | Out-Null

$testTable = if (Test-Path "_test_tables.json") {
    $data = Get-Content "_test_tables.json" -Raw | ConvertFrom-Json
    "$($data.tables[0].schema).$($data.tables[0].tableName)"
} else { $null }

$testView = if (Test-Path "_test_views.json") {
    $data = Get-Content "_test_views.json" -Raw | ConvertFrom-Json
    "$($data.views[0].schema).$($data.views[0].viewName)"
} else { $null }

$testProc = if (Test-Path "_test_procedures.json") {
    $data = Get-Content "_test_procedures.json" -Raw | ConvertFrom-Json
    "$($data.procedures[0].schema).$($data.procedures[0].procedureName)"
} else { $null }

$testFunc = if (Test-Path "_test_functions.json") {
    $data = Get-Content "_test_functions.json" -Raw | ConvertFrom-Json
    "$($data.functions[0].schema).$($data.functions[0].functionName)"
} else { $null }

$testTrig = if (Test-Path "_test_triggers.json") {
    $data = Get-Content "_test_triggers.json" -Raw | ConvertFrom-Json
    if ($data.triggers.Count -gt 0) {
        "$($data.triggers[0].schema).$($data.triggers[0].triggerName)"
    } else { $null }
} else { $null }

$testPkg = if (Test-Path "_test_packages.json") {
    $data = Get-Content "_test_packages.json" -Raw | ConvertFrom-Json
...
foreach ($query in $queries) {
    ...
    if ($queryToCliMap.ContainsKey($queryName)) {
        ...
        if ($cliObj -eq $null -and $cliSchema -eq $null) {
            ...
            & $exe --profile $Profile --command $cliCmd --outfile "_query_test_$queryName.json" 2>&1 | Out-Null
        } elseif ($cliObj) {
            # Object-based command
            Write-Host "   Executing: $cliCmd --object $cliObj" -ForegroundColor Cyan
            & $exe --profile $Profile --command $cliCmd --object $cliObj --outfile "_query_test_$queryName.json" 2>&1 | Out-Null
        } elseif ($cliSchema) {
            # Schema-based command
            Write-Host "   Executing: $cliCmd --schema $cliSchema" -ForegroundColor Cyan
            & $exe --profile $Profile --command $cliCmd --schema $cliSchema --outfile "_query_test_$queryName.json" 2>&1 | Out-Null
        }
...
$results | ConvertTo-Json -Depth 5 | Out-File "_comprehensive_validation_results.json" -Encoding UTF8

Write-Host "📊 Results saved to: _comprehensive_validation_results.json" -ForegroundColor Cyan
Write-Host ""
